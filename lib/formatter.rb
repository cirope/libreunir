  ...
  def self.parse_utf8_files
    Dir.glob(File.expand_path("private/data") + '/' + "*.csv").each do |file|
      ...
      csv = SmarterCSV.process(file, 
        ...
        row = row.first
        attributes = (row.slice *(klass.accessible_attributes.map(&:to_sym)))
        ...
        if (association = APP_CONFIG["field_maps_#{APP_CONFIG['table_maps'][filename]}"][:many_to_one_association]).present?
          association.each do |k,v|
            ...
            k.each do |field|
              association_attributes = { :"#{association_key[v]}" => row[association_key[v].to_sym], :"#{v}" => row[field.to_sym] }
              association_klass.create!(association_attributes)
            end
          end
        end
      end
